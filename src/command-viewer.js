...
function logError(context, message) {
	process.stderr.write(
		`${colors.yellow}[${context}] ${message}${colors.reset}\n`,
	);
}
	...
	switch (entry.type) {
		case "cmd": {
			...
			if (entry.num === currentCommandNum) {
				...
				logError(
					"cmd",
					`Command ${entry.num} arrived after currentCommandNum ${currentCommandNum}`,
				);
			}
			break;
		}
		...
		case "stderr": {
			...
			if (cmd) {
				...
				logError(
					entry.type,
					`Received ${entry.type} for unknown command ${entry.num}`,
				);
			}
			break;
		}
		...
		case "exit": {
			...
			if (cmd) {
				...
				logError(
					"exit",
					`Received exit for unknown command ${entry.num} with code ${entry.exitCode}`,
				);
			}
			break;
		}
		...
		case "error": {
			...
			console.error(
				`${colors.red}❌ ${entry.context}: ${entry.message}${colors.reset}`,
			);
			break;
		}
...
console.log(
	colors.green +
		"║" +
		colors.reset +
		title.padStart((boxWidth + title.length) / 2).padEnd(boxWidth - 2) +
		colors.green +
		"║" +
		colors.reset,
);
